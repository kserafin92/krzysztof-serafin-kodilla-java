...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        // Given
        User user1 = new User("user1", "Real Name 1");
        User user2 = new User("user2", "Real Name 2");

        TaskList taskListInProgress = new TaskList("In Progress");
        Task task1 = new Task("Task 1", "Description 1", user1, user1, LocalDate.now().minusDays(5), LocalDate.now().plusDays(5));
        Task task2 = new Task("Task 2", "Description 2", user2, user2, LocalDate.now().minusDays(10), LocalDate.now().plusDays(10));
        taskListInProgress.addTask(task1);
        taskListInProgress.addTask(task2);

        TaskList taskListCompleted = new TaskList("Completed");
        Task task3 = new Task("Task 3", "Description 3", user1, user1, LocalDate.now().minusDays(15), LocalDate.now().minusDays(5));
        Task task4 = new Task("Task 4", "Description 4", user2, user2, LocalDate.now().minusDays(20), LocalDate.now().minusDays(10));
        taskListCompleted.addTask(task3);
        taskListCompleted.addTask(task4);

        Board board = new Board("My Board");
        board.addTaskList(taskListInProgress);
        board.addTaskList(taskListCompleted);

        // When
        double averageWorkingOnTaskDays = calculateAverageWorkingOnTaskDays(board);

        // Then
        assertEquals(7.5, averageWorkingOnTaskDays, 0.001);
    }

    private double calculateAverageWorkingOnTaskDays(Board board) {
